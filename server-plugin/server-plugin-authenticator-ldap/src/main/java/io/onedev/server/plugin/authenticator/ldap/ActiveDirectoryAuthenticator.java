...
public class ActiveDirectoryAuthenticator extends LdapAuthenticator {
    ...
    @Editable(order=100, name="LDAP URL", description = "" +
			...
			"<a href='https://docs.onedev.io/administration-guide/trust-self-signed-certificates' target='_blank'>configure SCMDev to trust the certificate</a>")
    @NotEmpty
	@Override
	...
	@Editable(order=300, description=""
			+ "Specify manager DN to authenticate SCMDev itself to Active Directory. The manager DN should be specified "
	...
	@Editable(order=1000, placeholder="Do not retrieve groups", description=""
			...
			+ "permissions to a Active Directory group, a SCMDev group with same name should be defined. "
			+ "Leave empty to manage group memberships at SCMDev side")
